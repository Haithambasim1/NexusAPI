...
const getCompanyInfo = (domain) => {
  ...
  if (/tech|software|code|app|web|net|cloud|data|compute|digital|cyber|ai/.test(baseName)) {
    industry = 'Technology';
    description = 'A technology company focused on innovative software solutions and digital transformation.';
    employeeRange = [50, 5000];
    foundedRange = [1995, 2020];
  } else if (/shop|store|market|buy|sell|retail|commerce/.test(baseName)) {
    industry = 'Retail & E-Commerce';
    description = 'A retail company offering a wide range of consumer products through online and physical channels.';
    employeeRange = [20, 2000];
    foundedRange = [1980, 2018];
  } else if (/food|eat|cuisine|meal|chef|kitchen|cook|dining/.test(baseName)) {
    industry = 'Food & Beverage';
    description = 'A food service company providing quality dining experiences and innovative culinary solutions.';
    employeeRange = [10, 500];
    foundedRange = [1970, 2019];
  } else if (/health|med|care|pharma|doctor|clinic|therapy/.test(baseName)) {
    industry = 'Healthcare';
    description = 'A healthcare provider focused on patient care, medical services, and health technology solutions.';
    employeeRange = [50, 10000];
    foundedRange = [1960, 2015];
  } else if (/travel|tour|journey|trip|vacation|visit/.test(baseName)) {
    industry = 'Travel & Hospitality';
    description = 'A travel service provider offering comprehensive solutions for leisure and business travelers.';
    employeeRange = [15, 1000];
    foundedRange = [1975, 2018];
  } else if (/edu|learn|teach|school|academy|training/.test(baseName)) {
    industry = 'Education';
    description = 'An educational institution providing learning opportunities and professional development programs.';
    ...
    description = 'A financial services company providing investment, banking, and wealth management solutions.';
    employeeRange = [50, 5000];
    foundedRange = [1950, 2010];
  } else if (/media|news|press|blog|journal|publish/.test(baseName)) {
    industry = 'Media & Communications';
    description = 'A media company delivering content, news, and information across multiple platforms.';
    employeeRange = [20, 1000];
    foundedRange = [1960, 2018];
  } else {
    // Default for unknown patterns
    const industries = ['Business Services', 'Manufacturing', 'Energy', 'Real Estate', 'Entertainment', 'Consulting'];
  ...
  const employeeCount = Math.floor(Math.random() * (employeeRange[1] - employeeRange[0])) + employeeRange[0];
  
  // Generate realistic founding year
  const foundedYear = Math.floor(Math.random() * (foundedRange[1] - foundedRange[0])) + foundedRange[0];
...
const generateTechnologies = (industry) => {
  ...
  const industryTech = {
    ...
    'Financial Services': ['Bloomberg Terminal', 'Salesforce Financial Services', 'Quickbooks', 'SAP'],
